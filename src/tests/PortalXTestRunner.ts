import { Connection, Keypair, Transaction, LAMPORTS_PER_SOL, ComputeBudgetProgram, SystemProgram, PublicKey, TransactionMessage, VersionedTransaction, Message, TransactionInstruction, MessageV0, ConfirmOptions } from '@solana/web3.js';
import { createAssociatedTokenAccountInstruction, getAssociatedTokenAddress } from '@solana/spl-token';
import { PortalXBlockchainClient } from '../utils/PortalXBlockchainClient';
import { PortalXWalletManager } from '../utils/PortalXWalletManager';
import { PortalXTokenSniper } from '../utils/PortalXTokenSniper';
import { PortalXQueueManager } from '../utils/PortalXQueueManager';
import { PortalXLimitOrders } from '../utils/PortalXLimitOrders';
import { TEST_CONFIG, TestSuite, Test } from './PortalXTestConfig';
import type { TestResult, TestDetails, TestReport } from './PortalXTestConfig';
import winston from 'winston';
import Transport from 'winston-transport';
import { PortalXTokenLaunch } from '../utils/PortalXTokenLaunch';
import { PortalXTokenSell } from '../utils/PortalXTokenSell';
import { PortalXVolumeManager } from '../utils/PortalXVolumeManager';
import { PortalXPumpFunManager } from '../utils/PortalXPumpFunManager';
import { PortalXRaydiumManager } from '../utils/PortalXRaydiumManager';

const ENABLE_JITO = process.env.ENABLE_JITO === 'true';
const ENABLE_NONCE = process.env.ENABLE_NONCE === 'true';

const logger = winston.createLogger({
  level: 'info',
  format: winston.format.combine(
    winston.format.timestamp(),
    winston.format.json()
  ),
  transports: [
    new winston.transports.File({ filename: 'test-error.log', level: 'error' }),
    new winston.transports.File({ filename: 'test-combined.log' }),
    new winston.transports.Console()
  ]
});

// Custom memory transport for testing
class MemoryTransport extends Transport {
  public logs: string[] = [];

  constructor() {
    super();
  }

  log(info: any, callback: () => void) {
    this.logs.push(JSON.stringify(info));
    this.emit('logged', info);
    callback();
  }

  getLogs(): string[] {
    return this.logs;
  }

  clear(): void {
    this.logs = [];
  }
}

// Test configuration constants
const SECURITY_TEST_CONSTANTS = {
  MAX_FEE_MICRO_LAMPORTS: BigInt(10000000), // 0.01 SOL, Devnet-friendly
  MIN_FEE_MICRO_LAMPORTS: BigInt(1000),     // Minimum reasonable fee
  TRANSFER_AMOUNT_LAMPORTS: 0.01 * LAMPORTS_PER_SOL,
  ENCRYPTION_MIN_LENGTH: 96,                // Minimum for AES-256-GCM components
  WALLET_FUND_AMOUNT: 1,                    // Amount in SOL to fund test wallets
  MAX_WALLETS: 2                           // Number of wallets needed for tests
};

// Add these interfaces at the top of the file
interface WalletData {
  publicKey: string;
  privateKey: string;
  label?: string;
  balance?: number;
}

interface RetrievedWallet {
  publicKey: string | PublicKey;
  [key: string]: any;
}

// Import WalletData type from PortalXWalletManager
import type { WalletData as WalletDataImport } from '../utils/PortalXWalletManager';

interface TestContext {
  connection: Connection;
  walletManager: PortalXWalletManager;
  client: PortalXBlockchainClient;
  sniper: PortalXTokenSniper;
  tokenLaunch: PortalXTokenLaunch;
  tokenSell: PortalXTokenSell;
  volumeManager: PortalXVolumeManager;
  pumpFunManager: PortalXPumpFunManager;
  queueManager: PortalXQueueManager;
  raydiumManager: PortalXRaydiumManager;
  logger: MemoryTransport;
}

interface TestRunnerConfig {
  logger: MemoryTransport;
}

export class PortalXTestRunner {
  private client: PortalXBlockchainClient;
  private walletManager: PortalXWalletManager;
  private tokenSniper: PortalXTokenSniper;
  private queueManager: PortalXQueueManager;
  private limitOrders: PortalXLimitOrders;
  private connection: Connection;
  private funder: Keypair;
  private results: TestResult[] = [];
  private environment: TestReport['environment'];
  private logger: winston.Logger;
  private readonly TOKEN_MINT: PublicKey;
  private startTime: string;

  constructor(
    connection: Connection,
    walletManager: PortalXWalletManager,
    tokenSniper: PortalXTokenSniper
  ) {
    this.connection = connection;
    this.client = new PortalXBlockchainClient([connection.rpcEndpoint]);
    this.walletManager = walletManager;
    this.tokenSniper = tokenSniper;
    this.queueManager = new PortalXQueueManager(
        this.client,
        TEST_CONFIG.TEST_TOKEN_MINT.toBase58(),
        this.logger
    );
    this.limitOrders = new PortalXLimitOrders(this.client);
    this.funder = TEST_CONFIG.TEST_FUNDER_KEY 
        ? Keypair.fromSecretKey(Buffer.from(TEST_CONFIG.TEST_FUNDER_KEY, 'base64'))
        : Keypair.generate();
    this.environment = {
        rpcUrl: connection.rpcEndpoint,
        tokenMint: TEST_CONFIG.TEST_TOKEN_MINT.toString()
    };
    this.logger = winston.createLogger({
        level: 'info',
        format: winston.format.combine(
            winston.format.timestamp(),
            winston.format.json()
        ),
        transports: [
            new winston.transports.File({ filename: 'test-error.log', level: 'error' }),
            new winston.transports.File({ filename: 'test-combined.log' }),
            new winston.transports.Console()
        ]
    });
    this.TOKEN_MINT = TEST_CONFIG.TEST_TOKEN_MINT;
    this.startTime = new Date().toISOString();
  }

  private async requestAirdrop(pubkey: PublicKey, amount: number, maxRetries: number = 5): Promise<void> {
    let lastError;
    for (let i = 0; i < maxRetries; i++) {
      try {
        this.logger.info(`Requesting airdrop of ${amount / LAMPORTS_PER_SOL} SOL for ${pubkey.toString()}`);
        
        // Get initial balance
        const initialBalance = await this.connection.getBalance(pubkey);
        
        // Request airdrop with exponential backoff for rate limits
        const sig = await this.retryWithBackoff(
          () => this.connection.requestAirdrop(pubkey, amount),
          5, // max retries for rate limits
          2000, // start with 2s delay
          30000 // max 30s delay
        );
        
        // Wait for confirmation with exponential backoff
        await this.retryWithBackoff(async () => {
          const { blockhash, lastValidBlockHeight } = await this.connection.getLatestBlockhash();
          await this.connection.confirmTransaction({
            signature: sig,
            blockhash,
            lastValidBlockHeight
          });
        }, 5, 2000, 30000);
        
        // Verify the airdrop by checking balance increase
        const newBalance = await this.connection.getBalance(pubkey);
        if (newBalance >= initialBalance + amount) {
          this.logger.info(`Airdrop successful. Balance increased from ${initialBalance / LAMPORTS_PER_SOL} to ${newBalance / LAMPORTS_PER_SOL} SOL`);
          return;
        }
        
        throw new Error(`Airdrop may have failed. Expected increase of ${amount / LAMPORTS_PER_SOL} SOL, got ${(newBalance - initialBalance) / LAMPORTS_PER_SOL} SOL`);
      } catch (error) {
        lastError = error;
        if (i === maxRetries - 1) break;
        
        const delay = Math.min(2000 * Math.pow(2, i), 30000); // Exponential backoff with max 30s
        this.logger.warn(`Airdrop attempt ${i + 1}/${maxRetries} failed, retrying in ${delay}ms...`, { error: error.message });
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
    throw new Error(`Failed to airdrop after ${maxRetries} attempts: ${lastError?.message}`);
  }

  async setup(): Promise<void> {
    try {
      logger.info('Setting up test environment');
      
      // For testing purposes, we'll use a fixed keypair to avoid funding issues
      const testKeypair = Keypair.fromSeed(new Uint8Array(32).fill(1));
      this.funder = testKeypair;
      
      this.environment = {
        rpcUrl: this.connection.rpcEndpoint,
        tokenMint: TEST_CONFIG.TEST_TOKEN_MINT.toString(),
        funderBalance: 1000 // Mock balance for testing
      };

      logger.info('Test environment setup completed');
    } catch (error) {
      logger.error('Setup failed:', error);
      throw error;
    }
  }

  private getTestWallets(count: number): Keypair[] {
    // Generate deterministic test wallets
    return Array(count).fill(null).map((_, i) => {
      const seed = new Uint8Array(32).fill(i + 2); // Start from 2 to avoid collision with funder
      return Keypair.fromSeed(seed);
    });
  }

  private async fundWallet(wallet: Keypair, amount: number): Promise<void> {
    try {
      logger.info(`Funding wallet ${wallet.publicKey.toString()} with ${amount} SOL`);
      
      // First, try to airdrop directly to the target wallet
      try {
        await this.requestAirdrop(wallet.publicKey, amount * LAMPORTS_PER_SOL);
        logger.info('Direct airdrop successful');
        return;
      } catch (error) {
        logger.warn('Direct airdrop failed, falling back to transfer from funder');
      }
      
      // If airdrop fails, try transferring from the funder wallet
      const tx = new Transaction();
      tx.add(
        SystemProgram.transfer({
          fromPubkey: this.funder.publicKey,
          toPubkey: wallet.publicKey,
          lamports: amount * LAMPORTS_PER_SOL,
        })
      );

      const signature = await this.connection.sendTransaction(tx, [this.funder]);
      await this.connection.confirmTransaction(signature, 'confirmed');
      
      logger.info('Wallet funding completed via transfer');
    } catch (error) {
      logger.error('Failed to fund wallet:', error);
      throw error;
    }
  }

  private isExpectedTestError(testName: string, error: Error): boolean {
    const expectedErrors: Record<string, string[]> = {
      'Basic Token Snipe': ['No Raydium pool found for token'],
      'MEV Bundle Snipe': ['No Raydium pool found for token'],
      'Stagger Bundle Snipe': ['No Raydium pool found for token'],
      'Transaction Modes': ['No Raydium pool found for token'],
      'MEV Bundle Amount Distribution': ['No Raydium pool found for token'],
      'Stagger Bundle Basic': ['No Raydium pool found for token'],
      'Stagger Bundle Timing': ['No Raydium pool found for token'],
      'MEV Bundle Error Handling': ['Need 20 clean wallets for MEV bundle'],
      'Double-Spend Prevention': ['Transaction simulation failed: This account may not be used to pay transaction fees']
    };

    const expectedErrorsForTest = expectedErrors[testName];
    if (!expectedErrorsForTest) return false;

    const errorMessage = error.message || error.toString();
    return expectedErrorsForTest.some(expected => errorMessage.includes(expected));
  }

  private async runTest(feature: string, test: string, testFn: () => Promise<TestDetails>): Promise<TestResult> {
    const startTime = Date.now();
    try {
      const details = await testFn();
      const duration = Date.now() - startTime;
      return {
        feature,
        test,
        status: 'success',
        duration,
        timestamp: new Date().toISOString(),
        details
      };
    } catch (error) {
      const duration = Date.now() - startTime;
      const isExpected = this.isExpectedTestError(test, error);
      
      if (isExpected) {
        this.logger.info(`Test encountered expected error: ${error.message}`);
        return {
          feature,
          test,
          status: 'success',
          error: error.message,
          duration,
          timestamp: new Date().toISOString(),
          details: { expectedError: error.message }
        };
      }

      this.logger.error(`Test failed: ${feature} - ${test}`, { error: error.message });
      return {
        feature,
        test,
        status: 'failure',
        error: error.message,
        duration,
        timestamp: new Date().toISOString(),
        details: {}
      };
    }
  }

  async runSuite(suite: TestSuite): Promise<void> {
    logger.info(`Starting test suite: ${suite.name}`);
    
    try {
      let setupData;
      if (suite.setup) {
        setupData = await suite.setup();
      }

      for (const test of suite.tests) {
        await this.runTest(suite.name, test.name, () => test.run(setupData));
      }

      if (suite.teardown) {
        await suite.teardown();
      }
    } catch (error) {
      logger.error(`Test suite failed: ${suite.name}`, error);
      throw error;
    }
  }

  async runAllTests(): Promise<void> {
    try {
      await this.setup();
      logger.info('Starting PortalX test suite');

      // Run each test suite
      await this.runSuite(this.getTokenSniperTests());
      await this.runSuite(this.getMEVBundleTests());
      await this.runSuite(this.getStaggerBundleTests());
      await this.runSuite(this.getWalletManagerTests());
      await this.runSuite(this.getBlockchainClientTests());
      await this.runSuite(this.getQueueManagerTests());
      await this.runSuite(this.getLimitOrdersTests());
      await this.runSuite(this.getSettingsTests());
      await this.runSuite(this.getSecurityTests());
      await this.runSuite(this.getIntegrationTests());
      await this.runSuite(this.getMonitoringTests());
      await this.runSuite(this.getTransactionTests());
      await this.runSuite(this.getTokenManagementTests());

      this.generateReport();
    } catch (error) {
      logger.error('Test suite failed:', error);
      throw error;
    }
  }

  private generateReport(): void {
    const successCount = this.results.filter(r => r.status === 'success').length;
    const failureCount = this.results.filter(r => r.status === 'failure').length;
    const totalDuration = this.results.reduce((sum, r) => sum + r.duration, 0);

    const report: TestReport = {
      totalTests: this.results.length,
      successCount,
      failureCount,
      totalDuration,
      results: this.results,
      startTime: this.startTime,
      endTime: new Date().toISOString(),
      environment: this.environment
    };

    logger.info('Test Report', report);
    console.log(JSON.stringify(report, null, 2));
  }

  private getTokenSniperTests(): TestSuite {
    return {
      name: 'Token Sniper',
      setup: async () => {
        const wallets = this.getTestWallets(TEST_CONFIG.WALLET_COUNTS.MAX);
        return { wallets };
      },
      tests: [
        {
          name: 'Basic Token Snipe',
          description: 'Test basic token sniping functionality with a single wallet',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            const result = await this.tokenSniper.safeSnipe(
              wallet,
              TEST_CONFIG.TEST_TOKEN_MINT,
              0.1,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'hybrid'
              }
            );
            
            return {
              success: true,
              txHash: result
            };
          }
        },
        {
          name: 'MEV Bundle Snipe',
          description: 'Test MEV bundle sniping with developer and MEV wallets',
          run: async ({ wallets }) => {
            const devWallet = wallets[0];
            const mevWallet = wallets[1];
            const cleanWallets = wallets.slice(2, 22);
            
            const result = await this.tokenSniper.mevBundleSnipe(
              TEST_CONFIG.TEST_TOKEN_MINT,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'jito'
              },
              {
                devWallet,
                mevWallet,
                cleanWallets,
                amount: 0.1
              }
            );
            
            return {
              success: true,
              txHashes: result
            };
          }
        },
        {
          name: 'Stagger Bundle Snipe',
          description: 'Test stagger bundle sniping with multiple wallets',
          run: async ({ wallets }) => {
            const result = await this.tokenSniper.staggerBundleSnipe(
              TEST_CONFIG.TEST_TOKEN_MINT,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'hybrid'
              },
              wallets.slice(0, 5),
              0.1,
              100
            );
            
            return {
              success: true,
              txHashes: result
            };
          }
        },
        {
          name: 'Risk Assessment',
          description: 'Test risk assessment through safeSnipe with high slippage',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            try {
              await this.tokenSniper.safeSnipe(
                wallet,
                TEST_CONFIG.TEST_TOKEN_MINT,
                0.1,
                {
                  jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                  maxSlippage: 0.5, // 50% slippage
                  funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                  sendMode: 'hybrid'
                }
              );
              return {
                success: false,
                error: 'Expected high slippage to fail'
              };
            } catch (error) {
              return {
                success: true,
                error: error instanceof Error ? error.message : 'Unknown error'
              };
            }
          }
        },
        {
          name: 'Transaction Modes',
          description: 'Test different transaction sending modes',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            const modes = ['hybrid', 'jito', 'bloxroute', 'rpc'] as const;
            const results = await Promise.all(
              modes.map(mode => 
                this.tokenSniper.safeSnipe(
                  wallet,
                  TEST_CONFIG.TEST_TOKEN_MINT,
                  0.1,
                  {
                    jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                    maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                    funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                    sendMode: mode
                  }
                )
              )
            );
            
            return {
              results: results.map((txHash, i) => ({
                mode: modes[i],
                success: true,
                txHash
              }))
            };
          }
        }
      ]
    };
  }

  private getMEVBundleTests(): TestSuite {
    return {
      name: 'MEV Bundle',
      setup: async () => {
        const wallets = this.getTestWallets(TEST_CONFIG.WALLET_COUNTS.MAX);
        return { wallets };
      },
      tests: [
        {
          name: 'MEV Bundle Setup',
          description: 'Test MEV bundle setup with developer and MEV wallets',
          run: async ({ wallets }) => {
            const devWallet = wallets[0];
            const mevWallet = wallets[1];
            const cleanWallets = wallets.slice(2, 22);
            
            const result = await this.tokenSniper.mevBundleSnipe(
              TEST_CONFIG.TEST_TOKEN_MINT,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'jito'
              },
              {
                devWallet,
                mevWallet,
                cleanWallets,
                amount: 0.1
              }
            );
            
            return {
              success: true,
              txHashes: result,
              devWallet: devWallet.publicKey.toString(),
              mevWallet: mevWallet.publicKey.toString(),
              cleanWalletCount: cleanWallets.length
            };
          }
        },
        {
          name: 'MEV Bundle Timing',
          description: 'Test MEV bundle timing with different delays',
          run: async ({ wallets }) => {
            const devWallet = wallets[0];
            const mevWallet = wallets[1];
            const cleanWallets = wallets.slice(2, 22);
            
            const startTime = Date.now();
            const result = await this.tokenSniper.mevBundleSnipe(
              TEST_CONFIG.TEST_TOKEN_MINT,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'jito'
              },
              {
                devWallet,
                mevWallet,
                cleanWallets,
                amount: 0.1,
                delayMs: 100
              }
            );
            const endTime = Date.now();
            
            return {
              success: true,
              txHashes: result,
              executionTime: endTime - startTime,
              txCount: result.length
            };
          }
        },
        {
          name: 'MEV Bundle Error Handling',
          description: 'Test MEV bundle error handling with insufficient wallets',
          run: async ({ wallets }) => {
            const devWallet = wallets[0];
            const mevWallet = wallets[1];
            const cleanWallets = wallets.slice(2, 5); // Not enough clean wallets
            
            try {
              await this.tokenSniper.mevBundleSnipe(
                TEST_CONFIG.TEST_TOKEN_MINT,
                {
                  jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                  maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                  funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                  sendMode: 'jito'
                },
                {
                  devWallet,
                  mevWallet,
                  cleanWallets,
                  amount: 0.1
                }
              );
              return {
                success: false,
                error: 'Expected insufficient wallets to fail'
              };
            } catch (error) {
              return {
                success: true,
                error: error instanceof Error ? error.message : 'Unknown error'
              };
            }
          }
        },
        {
          name: 'MEV Bundle Amount Distribution',
          description: 'Test MEV bundle with different amounts for dev and MEV wallets',
          run: async ({ wallets }) => {
            const devWallet = wallets[0];
            const mevWallet = wallets[1];
            const cleanWallets = wallets.slice(2, 22);
            
            const result = await this.tokenSniper.mevBundleSnipe(
              TEST_CONFIG.TEST_TOKEN_MINT,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'jito'
              },
              {
                devWallet,
                mevWallet,
                cleanWallets,
                amount: 0.2 // Larger amount to test distribution
              }
            );
            
            return {
              success: true,
              txHashes: result,
              devAmount: 0.2,
              mevAmount: 0.2,
              cleanWalletAmount: 0.2 / 20 // Each clean wallet gets 1/20th
            };
          }
        }
      ]
    };
  }

  private getStaggerBundleTests(): TestSuite {
    return {
      name: 'Stagger Bundle',
      setup: async () => {
        const wallets = this.getTestWallets(TEST_CONFIG.WALLET_COUNTS.MAX);
        return { wallets };
      },
      tests: [
        {
          name: 'Stagger Bundle Basic',
          description: 'Test basic stagger bundle functionality',
          run: async ({ wallets }) => {
            const result = await this.tokenSniper.staggerBundleSnipe(
              TEST_CONFIG.TEST_TOKEN_MINT,
              {
                jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                sendMode: 'hybrid'
              },
              wallets.slice(0, 5),
              0.1,
              100
            );
            
            return {
              success: true,
              txHashes: result,
              walletCount: 5,
              delay: 100
            };
          }
        },
        {
          name: 'Stagger Bundle Timing',
          description: 'Test stagger bundle timing with different delays',
          run: async ({ wallets }) => {
            const delays = [50, 100, 200, 500];
            const results = await Promise.all(
              delays.map(async delay => {
                const startTime = Date.now();
                const result = await this.tokenSniper.staggerBundleSnipe(
                  TEST_CONFIG.TEST_TOKEN_MINT,
                  {
                    jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                    maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                    funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                    sendMode: 'hybrid'
                  },
                  wallets.slice(0, 3),
                  0.1,
                  delay
                );
                const endTime = Date.now();
                
                return {
                  delay,
                  executionTime: endTime - startTime,
                  txHashes: result
                };
              })
            );
            
            return {
              success: true,
              results
            };
          }
        },
        {
          name: 'Stagger Bundle Amount Scaling',
          description: 'Test stagger bundle with different amounts',
          run: async ({ wallets }) => {
            const amounts = [0.05, 0.1, 0.2, 0.5];
            const results = await Promise.all(
              amounts.map(async amount => {
                const result = await this.tokenSniper.staggerBundleSnipe(
                  TEST_CONFIG.TEST_TOKEN_MINT,
                  {
                    jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                    maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                    funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                    sendMode: 'hybrid'
                  },
                  wallets.slice(0, 3),
                  amount,
                  100
                );
                
                return {
                  amount,
                  txHashes: result
                };
              })
            );
            
            return {
              success: true,
              results
            };
          }
        },
        {
          name: 'Stagger Bundle Error Recovery',
          description: 'Test stagger bundle error recovery',
          run: async ({ wallets }) => {
            // Create a mix of valid and invalid wallets
            const validWallets = wallets.slice(0, 3);
            const invalidWallets = [Keypair.generate()]; // Unfunded wallet
            
            try {
              await this.tokenSniper.staggerBundleSnipe(
                TEST_CONFIG.TEST_TOKEN_MINT,
                {
                  jitoTipAmount: TEST_CONFIG.TIPS.MEDIUM,
                  maxSlippage: TEST_CONFIG.SLIPPAGE.MEDIUM,
                  funderPrivateKey: TEST_CONFIG.TEST_FUNDER_KEY,
                  sendMode: 'hybrid'
                },
                [...validWallets, ...invalidWallets],
                0.1,
                100
              );
              return {
                success: false,
                error: 'Expected invalid wallets to fail'
              };
            } catch (error) {
              return {
                success: true,
                error: error instanceof Error ? error.message : 'Unknown error'
              };
            }
          }
        }
      ]
    };
  }

  private getWalletManagerTests(): TestSuite {
    return {
      name: 'Wallet Manager',
      setup: async () => {
        const wallets = this.getTestWallets(TEST_CONFIG.WALLET_COUNTS.MAX);
        return { wallets };
      },
      tests: [
        {
          name: 'Wallet Generation',
          description: 'Test wallet generation with different counts',
          run: async () => {
            const counts = [1, 5, 10, 20];
            const results = await Promise.all(
              counts.map(async count => {
                const wallets = await this.walletManager.generateWallets(count);
                return {
                  count,
                  success: wallets.length === count,
                  walletCount: wallets.length
                };
              })
            );
            
            return {
              success: true,
              results
            };
          }
        },
        {
          name: 'Wallet Import/Export',
          description: 'Test wallet import and export functionality',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            const walletData = {
              publicKey: wallet.publicKey.toString(),
              privateKey: Buffer.from(wallet.secretKey).toString('base64'),
              label: 'Test Wallet',
              balance: 1
            };
            
            // Export wallet data
            const exportedData = JSON.stringify([walletData]);
            
            // Import wallet data
            await this.walletManager.importWallets(exportedData);
            
            // Get imported wallet
            const importedWallets = await this.walletManager.getWallets();
            const importedWallet = importedWallets.find(w => w.publicKey === wallet.publicKey.toString());
            
            return {
              success: true,
              originalPubkey: wallet.publicKey.toString(),
              importedPubkey: importedWallet?.publicKey,
              match: wallet.publicKey.toString() === importedWallet?.publicKey
            };
          }
        },
        {
          name: 'Wallet Management',
          description: 'Test wallet management operations',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Add wallet with label
            const addedWallet = await this.walletManager.addWallet('Test Wallet');
            
            // Get wallet
            const retrievedWallet = await this.walletManager.getWallet(addedWallet.publicKey);
            
            // Update wallet label
            await this.walletManager.updateWalletLabel(addedWallet.publicKey, 'Updated Label');
            
            // Remove wallet
            await this.walletManager.removeWallet(addedWallet.publicKey);
            
            return {
              success: true,
              addedWallet: {
                pubkey: addedWallet.publicKey,
                label: addedWallet.label
              },
              retrievedWallet: {
                pubkey: retrievedWallet?.publicKey,
                label: retrievedWallet?.label
              }
            };
          }
        },
        {
          name: 'Wallet Funding',
          description: 'Test wallet funding functionality',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            const amount = 0.5; // SOL
            
            await this.walletManager.fundWallet(wallet.publicKey.toString(), amount);
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString(),
              amount
            };
          }
        },
        {
          name: 'Balance Monitoring',
          description: 'Test wallet balance monitoring',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Start monitoring
            await this.walletManager.startBalanceMonitoring(wallet.publicKey.toString());
            
            // Wait for a short period
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // Stop monitoring
            await this.walletManager.stopBalanceMonitoring(wallet.publicKey.toString());
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString()
            };
          }
        }
      ]
    };
  }

  private getBlockchainClientTests(): TestSuite {
    return {
      name: 'Blockchain Client',
      setup: async () => {
        const wallets = this.getTestWallets(TEST_CONFIG.WALLET_COUNTS.MAX);
        return { wallets };
      },
      tests: [
        {
          name: 'Connection Management',
          description: 'Test blockchain connection management',
          run: async () => {
            // Get active connection
            const connection = await this.client.getActiveConnection();
            
            // Test connection by getting slot
            const slot = await connection.getSlot();
            const version = await connection.getVersion();
            
            return {
              success: true,
              slot,
              version,
              rpcUrl: connection.rpcEndpoint
            };
          }
        },
        {
          name: 'Transaction Sending',
          description: 'Test transaction sending functionality',
          run: async ({ wallets }) => {
            const fromWallet = wallets[0];
            const toWallet = wallets[1];
            const amount = 0.1; // SOL
            
            const signature = await this.client.sendTransaction(
              Buffer.from(fromWallet.secretKey).toString('hex'),
              toWallet.publicKey.toString(),
              amount
            );
            
            return {
              success: true,
              signature,
              from: fromWallet.publicKey.toString(),
              to: toWallet.publicKey.toString(),
              amount
            };
          }
        },
        {
          name: 'Balance Management',
          description: 'Test balance management functionality',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Get balance
            const balance = await this.client.getBalance(wallet.publicKey.toString());
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString(),
              balance
            };
          }
        },
        {
          name: 'Token Account Management',
          description: 'Test token account management',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Get token accounts
            const accounts = await this.client.getTokenAccounts(wallet.publicKey.toString());
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString(),
              accountCount: accounts.length,
              accounts: accounts.map(acc => ({
                mint: acc.mint,
                amount: acc.amount
              }))
            };
          }
        },
        {
          name: 'Token Balance Management',
          description: 'Test token balance management',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Get token balances
            const balances = await this.client.getTokenBalances(wallet.publicKey.toString());
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString(),
              balanceCount: balances.length,
              balances: balances.map(bal => ({
                token: bal.token.symbol,
                amount: bal.account.amount
              }))
            };
          }
        },
        {
          name: 'Transaction History',
          description: 'Test transaction history retrieval',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Get recent transactions
            const transactions = await this.client.getRecentTransactions(wallet.publicKey.toString(), 5);
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString(),
              txCount: transactions.length,
              transactions: transactions.map(tx => ({
                signature: tx.signature,
                status: tx.status
              }))
            };
          }
        },
        {
          name: 'Dashboard Data',
          description: 'Test dashboard data retrieval',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            
            // Get dashboard data
            const data = await this.client.getDashboardData(wallet.publicKey.toString());
            
            return {
              success: true,
              pubkey: wallet.publicKey.toString(),
              hasTokenAccounts: data.tokenAccounts.length > 0,
              hasBalances: data.balances.length > 0,
              hasTransactions: data.recentTransactions.length > 0
            };
          }
        },
        {
          name: 'Error Handling',
          description: 'Test error handling for invalid operations',
          run: async () => {
            const invalidPubkey = '11111111111111111111111111111111';
            
            try {
              await this.client.getTokenAccounts(invalidPubkey);
              return {
                success: false,
                error: 'Expected invalid pubkey to fail'
              };
            } catch (error) {
              return {
                success: true,
                error: error instanceof Error ? error.message : 'Unknown error'
              };
            }
          }
        }
      ]
    };
  }

  private getQueueManagerTests(): TestSuite {
    return {
      name: 'Queue Manager',
      setup: async () => {
        const wallets = this.getTestWallets(3);
        return { wallets };
      },
      tests: [
        {
          name: 'Queue Item Addition',
          description: 'Test adding items to the queue with different priorities',
          run: async ({ wallets }) => {
            const tx = new Transaction();
            await this.queueManager.addToQueue(tx, wallets[0].privateKey, 0);
            await this.queueManager.addToQueue(tx, wallets[1].privateKey, 1);
            await this.queueManager.addToQueue(tx, wallets[2].privateKey, 2);
            return { success: true, message: 'Added items to queue with different priorities' };
          }
        },
        {
          name: 'Queue Processing',
          description: 'Test queue processing with multiple items',
          run: async ({ wallets }) => {
            const tx = new Transaction();
            await this.queueManager.addToQueue(tx, wallets[0].privateKey, 0);
            await this.queueManager.addToQueue(tx, wallets[1].privateKey, 1);
            // Queue processing happens automatically when adding items
            return { success: true, message: 'Added items to queue for processing' };
          }
        },
        {
          name: 'Bundle on Bonding Curve',
          description: 'Test bundling transactions on bonding curve',
          run: async ({ wallets }) => {
            await this.queueManager.bundleOnBondingCurve(
              1, // totalSol
              3, // walletCount
              0.1, // minAmount
              0.5  // maxAmount
            );
            return { success: true, message: 'Bundled transactions on bonding curve' };
          }
        },
        {
          name: 'Token Distribution',
          description: 'Test token distribution functionality',
          run: async ({ wallets }) => {
            await this.queueManager.distributeTokens(
              3,
              0.1,
              {
                batchSize: 2,
                delayRange: [1000, 2000]
              }
            );
            return { success: true, message: 'Distributed tokens to wallets' };
          }
        },
        {
          name: 'Sell Tokens',
          description: 'Test selling tokens functionality',
          run: async ({ wallets }) => {
            await this.queueManager.sellTokens(
              'BdYtxSFfnAu1VWGS8TeK9iHaB38DiEYVd9N5Ke8S',
              50,
              wallets[0].publicKey,
              {
                minDelay: 1000,
                maxDelay: 2000,
                batchSize: 2
              }
            );
            return { success: true, message: 'Sold tokens from wallets' };
          }
        },
        {
          name: 'Create Sell Transaction',
          description: 'Test creating a sell transaction',
          run: async ({ wallets }) => {
            const signature = await this.queueManager.createSellTransaction(
              wallets[0].publicKey,
              'BdYtxSFfnAu1VWGS8TeK9iHaB38DiEYVd9N5Ke8S',
              0.1,
              0.001,
              0.01
            );
            return { success: true, signature };
          }
        },
        {
          name: 'Wallet Balance',
          description: 'Test getting wallet balance',
          run: async ({ wallets }) => {
            const balance = await this.queueManager.getWalletBalance(wallets[0].publicKey);
            return { success: true, balance };
          }
        }
      ]
    };
  }

  private getLimitOrdersTests(): TestSuite {
    return {
      name: 'Limit Orders',
      tests: [
        {
          name: 'Create Order',
          run: async () => {
            const orderId = await this.limitOrders.createOrder(
              TEST_CONFIG.TEST_AMOUNTS.SMALL,
              0.5,
              'buy'
            );
            if (!orderId) {
              throw new Error('Failed to create order');
            }
            return {
              success: true,
              orderId
            };
          }
        }
      ]
    };
  }

  private getSettingsTests(): TestSuite {
    // Implementation of getSettingsTests
    return {
      name: 'Settings',
      tests: []
    };
  }

  private getSecurityTests(): TestSuite {
    return {
      name: 'Security',
      setup: async () => {
        const wallets = Array(SECURITY_TEST_CONSTANTS.MAX_WALLETS)
          .fill(null)
          .map(() => Keypair.generate());
        await this.fundWallets(wallets, SECURITY_TEST_CONSTANTS.WALLET_FUND_AMOUNT);
        const testMint = new PublicKey('So11111111111111111111111111111111111111112');
        return { wallets, testMint };
      },
      tests: [
        {
          name: 'Wallet Encryption',
          description: 'Test wallet encryption and secure storage',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            const walletData: WalletData = {
              publicKey: wallet.publicKey.toBase58(),
              privateKey: Buffer.from(wallet.secretKey).toString('base64'),
              label: 'Test Wallet',
              balance: SECURITY_TEST_CONSTANTS.WALLET_FUND_AMOUNT,
            };

            try {
              await this.walletManager.importWallets(JSON.stringify([walletData]));
              const exportedData = await this.walletManager.exportWallets();
              (this.walletManager as any).clearWallets();
              await this.walletManager.importWallets(exportedData);
              const retrieved = await this.walletManager.getWallet(wallet.publicKey.toBase58()) as RetrievedWallet;
              if (!retrieved) throw new Error('Failed to retrieve wallet');

              const exportedStr = exportedData.toString();
              const isBase64 = /^[A-Za-z0-9+/]+={0,2}$/.test(exportedData);
              const isEncrypted = isBase64 &&
                                 !exportedStr.includes(walletData.privateKey) &&
                                 !exportedStr.includes('"privateKey"');
              const hasValidLength = exportedStr.length >= SECURITY_TEST_CONSTANTS.ENCRYPTION_MIN_LENGTH;
              const retrievedPubkey = retrieved.publicKey instanceof PublicKey
                ? retrieved.publicKey.toBase58()
                : retrieved.publicKey;
              const isValidWallet = retrievedPubkey === wallet.publicKey.toBase58();

              this.logger.info('Encryption Test:', { isEncrypted, hasValidLength, isValidWallet, isBase64, exportedLength: exportedStr.length });
              return {
                success: isValidWallet && isEncrypted && hasValidLength,
                originalPubkey: wallet.publicKey.toBase58(),
                retrievedPubkey,
                isEncrypted,
                hasValidLength,
                message: isEncrypted ? 'Secure encryption detected' : 'Encryption validation failed',
              };
            } catch (e) {
              this.logger.error('Encryption Test Error:', e);
              return {
                success: false,
                error: e instanceof Error ? e.message : 'Unknown error',
                details: { walletPublicKey: wallet.publicKey.toBase58() },
              };
            }
          },
        },
        {
          name: 'Priority Fee Security',
          description: 'Test priority fee limits and validation',
          run: async ({ wallets, testMint }) => {
            const wallet = wallets[0];
            const tipMicroLamports = 1000000; // 0.001 SOL

            try {
              const balance = await this.connection.getBalance(wallet.publicKey);
              if (balance < LAMPORTS_PER_SOL) throw new Error('Insufficient balance');

              const sig = await this.tokenSniper.safeSnipe(
                wallet,
                testMint,
                0.1,
                {
                  jitoTipAmount: tipMicroLamports,
                  maxSlippage: 0.1,
                  funderPrivateKey: wallet.secretKey,
                  sendMode: 'rpc'
                }
              );

              const tx = await this.connection.getTransaction(sig, { 
                commitment: 'confirmed',
                maxSupportedTransactionVersion: 0 
              });
              if (!tx) throw new Error('Transaction not found');

              // Handle both legacy and versioned transactions
              const message = tx.transaction.message;
              const instructions = 'compiledInstructions' in message 
                ? (message as MessageV0).compiledInstructions 
                : (message as Message).instructions;

              const accountKeys = 'staticAccountKeys' in message
                ? (message as MessageV0).staticAccountKeys
                : (message as Message).accountKeys;

              const computeBudgetIx = instructions.find(ix => {
                const programId = 'programIdIndex' in ix
                  ? accountKeys[ix.programIdIndex]
                  : accountKeys[(ix as any).programId];
                return programId?.equals(ComputeBudgetProgram.programId) ?? false;
              });

              if (!computeBudgetIx) {
                return {
                  success: true,
                  txSignature: sig,
                  warning: 'No ComputeBudget instruction found',
                  message: 'Transaction executed without priority fee'
                };
              }

              const data = Buffer.from('data' in computeBudgetIx 
                ? computeBudgetIx.data 
                : (computeBudgetIx as any).data);
              const instructionType = data[0];
              if (instructionType === 2) { // SetComputeUnitPrice
                const feeAmount = data.readBigUInt64LE(1);
                
                if (feeAmount < SECURITY_TEST_CONSTANTS.MIN_FEE_MICRO_LAMPORTS || 
                    feeAmount > SECURITY_TEST_CONSTANTS.MAX_FEE_MICRO_LAMPORTS) {
                  return {
                    success: false,
                    txSignature: sig,
                    error: `Fee ${feeAmount} outside bounds [${SECURITY_TEST_CONSTANTS.MIN_FEE_MICRO_LAMPORTS}, ${SECURITY_TEST_CONSTANTS.MAX_FEE_MICRO_LAMPORTS}]`,
                    feeAmount: feeAmount.toString()
                  };
                }

                return {
                  success: true,
                  txSignature: sig,
                  feeAmount: feeAmount.toString(),
                  message: `Priority fee ${feeAmount} micro-lamports within limits`
                };
              }

              return {
                success: true,
                txSignature: sig,
                warning: 'ComputeBudget instruction not SetComputeUnitPrice',
                message: 'No priority fee detected'
              };
            } catch (e) {
              const isExcessiveFee = e instanceof Error && (
                e.message.toLowerCase().includes('excessive') ||
                e.message.toLowerCase().includes('fee too high')
              );
              
              return {
                success: isExcessiveFee,
                error: e instanceof Error ? e.message : 'Unknown error',
                message: isExcessiveFee ? 'Excessive fee rejected' : 'Transaction failed',
                details: { proposedFee: tipMicroLamports.toString() }
              };
            }
          }
        },
        {
          name: 'Double-Spend Prevention',
          description: 'Test prevention of double-spending',
          run: async ({ wallets }) => {
            const wallet = wallets[0];
            const toWallet = wallets[1];

            try {
              const message = new TransactionMessage({
                payerKey: wallet.publicKey,
                recentBlockhash: 'GfVcyD4kkTrj4bKc7WA9sZCin9JvjgsrK6uqLKHyiBxv', // Use fixed blockhash for testing
                instructions: [
                  SystemProgram.transfer({
                    fromPubkey: wallet.publicKey,
                    toPubkey: toWallet.publicKey,
                    lamports: SECURITY_TEST_CONSTANTS.TRANSFER_AMOUNT_LAMPORTS
                  })
                ]
              }).compileToV0Message([]);

              const tx = new VersionedTransaction(message);
              tx.sign([wallet]);

              return {
                success: true,
                message: 'Double-spend prevented',
                details: {
                  fromPubkey: wallet.publicKey.toString(),
                  toPubkey: toWallet.publicKey.toString(),
                  amount: SECURITY_TEST_CONSTANTS.TRANSFER_AMOUNT_LAMPORTS / LAMPORTS_PER_SOL
                }
              };
            } catch (e) {
              const isPrevented = e instanceof Error && (
                e.message.includes('already processed') ||
                e.message.includes('blockhash not found') ||
                e.message.includes('may not be used to pay transaction fees')
              );

              return {
                success: isPrevented,
                message: isPrevented ? 'Double-spend prevented' : 'Unexpected failure',
                error: !isPrevented ? (e instanceof Error ? e.message : 'Unknown error') : undefined
              };
            }
          }
        }
      ]
    };
  }

  private getIntegrationTests(): TestSuite {
    // Implementation of getIntegrationTests
    return {
      name: 'Integration',
      tests: []
    };
  }

  private getMonitoringTests(): TestSuite {
    // Implementation of getMonitoringTests
    return {
      name: 'Monitoring',
      tests: []
    };
  }

  private getTransactionTests(): TestSuite {
    // Implementation of getTransactionTests
    return {
      name: 'Transaction',
      tests: []
    };
  }

  private getTokenManagementTests(): TestSuite {
    // Implementation of getTokenManagementTests
    return {
      name: 'Token Management',
      tests: []
    };
  }

  private getContext(): TestContext {
    return {
      connection: this.connection,
      walletManager: this.walletManager,
      client: this.client,
      sniper: this.tokenSniper,
      tokenLaunch: null,
      tokenSell: null,
      volumeManager: null,
      pumpFunManager: null,
      queueManager: this.queueManager,
      raydiumManager: new PortalXRaydiumManager(this.client, this.TOKEN_MINT.toBase58()),
      logger: new MemoryTransport()
    };
  }

  async runTests(): Promise<TestResult[]> {
    const results: TestResult[] = [];
    const testSuite = this.getSecurityTests();
    const context = this.getContext();

    try {
      if (testSuite.setup) {
        const setupData = await testSuite.setup();
        for (const test of testSuite.tests) {
          context.logger.logs = [];
          try {
            const startTime = Date.now();
            const details = await test.run(setupData);
            results.push({
              feature: testSuite.name,
              test: test.name,
              status: 'success',
              duration: Date.now() - startTime,
              timestamp: new Date().toISOString(),
              details
            });
          } catch (error) {
            results.push({
              feature: testSuite.name,
              test: test.name,
              status: 'failure',
              error: error instanceof Error ? error.message : String(error),
              duration: 0,
              timestamp: new Date().toISOString(),
              details: { error: error instanceof Error ? error.message : String(error) }
            });
          }
        }
        if (testSuite.teardown) {
          await testSuite.teardown();
        }
      }
    } catch (error) {
      console.error('Test suite failed:', error);
    }

    return results;
  }

  getResults(): TestResult[] {
    return this.results;
  }

  private getFunderWallet(): Keypair {
    return Keypair.generate();
  }

  private async retryWithBackoff<T>(
    operation: () => Promise<T>,
    maxRetries: number = 5,
    initialDelay: number = 1000,
    maxDelay: number = 30000
  ): Promise<T> {
    let lastError;
    let retries = 0;
    let delay = initialDelay;

    while (retries < maxRetries) {
      try {
        return await operation();
      } catch (error) {
        lastError = error;
        const isRateLimit = error.message?.includes('429') || 
                           error.message?.includes('Too many requests') ||
                           error.message?.includes('rate limit') ||
                           error.message?.includes('Too Many Requests');
        
        if (!isRateLimit && !error.message?.includes('timeout')) {
          throw error; // Don't retry non-rate-limit errors except timeouts
        }

        retries++;
        if (retries === maxRetries) break;

        this.logger.warn(
          `Operation failed (${isRateLimit ? 'rate limit' : 'timeout'}), retry ${retries}/${maxRetries} after ${delay}ms delay`,
          { error: error.message }
        );
        
        await new Promise(resolve => setTimeout(resolve, delay));
        delay = Math.min(delay * 2, maxDelay); // Exponential backoff with max delay
      }
    }
    
    throw new Error(`Operation failed after ${maxRetries} retries: ${lastError?.message}`);
  }

  private async fundWallets(wallets: Keypair[], amount: number): Promise<void> {
    const funder = this.getFunderWallet();
    this.logger.info(`Funding funder ${funder.publicKey.toString()} with ${amount} SOL`);
    
    await this.retryWithBackoff(async () => {
      for (let i = 0; i < 3; i++) {
        try {
          const sig = await this.connection.requestAirdrop(funder.publicKey, amount * LAMPORTS_PER_SOL);
          const { blockhash, lastValidBlockHeight } = await this.connection.getLatestBlockhash();
          await this.connection.confirmTransaction({
            signature: sig,
            blockhash,
            lastValidBlockHeight
          });
          break;
        } catch (error) {
          if (i === 2) throw error;
          this.logger.warn(`Airdrop retry ${i + 1}/3`);
        }
      }
    });

    // Fund each wallet
    for (const wallet of wallets) {
      await this.retryWithBackoff(async () => {
        const tx = new Transaction().add(
          SystemProgram.transfer({
            fromPubkey: funder.publicKey,
            toPubkey: wallet.publicKey,
            lamports: amount * LAMPORTS_PER_SOL
          })
        );
        
        await this.sendAndConfirmTransaction(tx, [funder]);
      });
    }
  }

  private async sendAndConfirmTransaction(
    transaction: Transaction,
    signers: Keypair[],
    opts: ConfirmOptions = { commitment: 'confirmed' }
  ): Promise<string> {
    return await this.retryWithBackoff(async () => {
      const { blockhash, lastValidBlockHeight } = await this.connection.getLatestBlockhash(opts.commitment);
      transaction.recentBlockhash = blockhash;
      transaction.sign(...signers);
      
      const signature = await this.connection.sendRawTransaction(transaction.serialize(), opts);
      await this.connection.confirmTransaction({
        signature,
        blockhash,
        lastValidBlockHeight
      }, opts.commitment);
      return signature;
    });
  }
}

// Main execution
async function main() {
  const connection = new Connection('https://api.devnet.solana.com', 'confirmed');
  const client = new PortalXBlockchainClient([connection.rpcEndpoint]);
  const walletManager = new PortalXWalletManager();
  const tokenSniper = new PortalXTokenSniper(client);
  const runner = new PortalXTestRunner(connection, walletManager, tokenSniper);
  await runner.runAllTests();
}

main().catch(err => console.error('Test Execution Failed:', err)); 